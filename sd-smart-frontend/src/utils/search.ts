...
export function matchProduct(product: any, query: string): boolean {
  ...
  if (product.category) searchableTexts.push(product.category.toLowerCase());
  if (product.categoryLabel) searchableTexts.push(product.categoryLabel.toLowerCase());
  if (product.modelNumber) searchableTexts.push(product.modelNumber.toLowerCase());
  if (product.sku) searchableTexts.push(product.sku.toLowerCase());
  if (product.productId) searchableTexts.push(product.productId.toLowerCase());
  if (product.badge) searchableTexts.push(product.badge.toLowerCase());
  if (product.eyebrow) searchableTexts.push(product.eyebrow.toLowerCase());
  if (product.description) searchableTexts.push(product.description.toLowerCase());
  if (product.productDescription) searchableTexts.push(product.productDescription.toLowerCase());
  if (product.warranty) searchableTexts.push(product.warranty.toLowerCase());
  if (product.href) searchableTexts.push(product.href.toLowerCase());
    ...
    try {
      ...
      if (Array.isArray(specsArr)) {
        specsArr.forEach((spec: any) => {
          if (spec.label) searchableTexts.push(spec.label.toLowerCase());
          if (spec.value) searchableTexts.push(spec.value.toLowerCase());
        });
      }
    } catch (e) {
