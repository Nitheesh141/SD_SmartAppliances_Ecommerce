    ...
    <div className={`relative flex flex-col md:flex-row items-center justify-between w-full mb-12 md:mb-16 last:mb-0 ${isLeft ? "" : "md:flex-row-reverse"}`}>
      ...
      <div className="w-full md:w-[45%] flex flex-col items-start px-2">
        <div className="bg-white border border-slate-100 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow duration-300 dark:bg-slate-900/50 dark:border-slate-800 dark:hover:shadow-none w-full">
          {/* Mobile Year Badge */}
          <span className="inline-flex md:hidden items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-teal-50 text-teal-700 mb-3 uppercase dark:bg-teal-950 dark:text-teal-400">
            {item.year}
          </span>
      ...
      <div className="absolute left-4 md:left-1/2 md:-translate-x-1/2 top-4 md:top-1/2 md:-translate-y-1/2 z-10 flex items-center justify-center">
        {/* Outer Circle */}
        <div className="h-8 w-8 rounded-full bg-teal-50 border-2 border-teal-500 flex items-center justify-center dark:bg-teal-950">
          {/* Inner Bullet */}
          <div className="h-3 w-3 rounded-full bg-teal-600" />
        </div>
        
        {/* Floating Year label for desktop */}
        <div className={`hidden md:block absolute whitespace-nowrap text-xl font-bold font-heading text-teal-600 dark:text-teal-400 ${isLeft ? "left-12" : "right-12"}`}>
          {item.year}
        </div>
      </div>
