...
export default function AdminServiceRequestsPage() {
  ...
  const handleStatusUpdate = async (id: string, newStatus: string) => {
    ...
    try {
      ...
      const res = await serviceRequestService.updateServiceRequestStatus(id, {
        status: newStatus,
  ...
  const filteredRequests = requests.filter(req => {
    ...
    switch (activeTab) {
      ...
      case "UNDER_SERVICE":
        return ["Product Collected", "Under Inspection", "Under Repair"].includes(req.currentStatus);
      case "COMPLETED":
        return ["Service Completed", "Ready For Delivery", "Delivered"].includes(req.currentStatus);
      case "CLOSED":
        return req.currentStatus === "Closed";
      case "ALL":
      default:
        return true;
    }
  ...
  const underServiceRequests = requests.filter(r => ["Product Collected", "Under Inspection", "Under Repair"].includes(r.currentStatus)).length;
  const completedRequests = requests.filter(r => ["Service Completed", "Ready For Delivery", "Delivered"].includes(r.currentStatus)).length;
  const closedRequests = requests.filter(r => r.currentStatus === "Closed").length;

  // Render Status Badge
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "Pending Verification":
        return <span className="inline-block whitespace-nowrap px-2.5 py-1 text-xs font-bold rounded-full bg-yellow-500/10 text-yellow-500">Pending Verification</span>;
      case "Verified":
  ...
  return (
    <div className={cn(
      ...
      <div className="flex-grow lg:pl-64 flex flex-col min-h-screen">
        {/* Main content area */}
        <main className="flex-grow flex flex-col min-w-0 h-screen overflow-y-auto px-6 sm:px-10 py-8 text-left">
        ...
        {loading ? (
          ...
          <div className="mt-6 space-y-4">
            {/* Desktop View: Table */}
            <div className={cn(
              "hidden lg:block rounded-2xl border overflow-hidden shadow-sm",
              isDark ? "bg-[#0d0d0d] border-neutral-800" : "bg-white border-neutral-200"
            )}>
                  ...
                  <tbody className={cn("divide-y text-xs", isDark ? "divide-neutral-800/80" : "divide-neutral-100")}>
                    {filteredRequests.map((req) => (
                      <tr key={req.id} className={cn("transition-colors", isDark ? "hover:bg-neutral-900/40" : "hover:bg-neutral-50/50")}>
                        <td className="px-6 py-4 font-mono font-bold text-[#D71920]">{req.ticketId}</td>
      ...
      {selectedRequest && (
        <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          ...
          {/* Drawer Body */}
          <div className={cn(
            "relative w-full max-w-2xl h-screen shadow-2xl flex flex-col text-left animate-in slide-in-from-right duration-350",
            isDark ? "bg-[#0d0d0d] border-l border-neutral-800 text-white" : "bg-white border-l border-neutral-200 text-slate-900"
          )}>
            
            {/* Header */}
            <div className={cn(
              "px-6 py-4 border-b flex items-center justify-between sticky top-0 z-10 backdrop-blur",
              ...
              <button
                ...
                className="p-1.5 rounded-lg border border-neutral-800 hover:bg-neutral-800/40 text-neutral-400 hover:text-white"
              >
                <X size={16} />
              </button>
            </div>
              ...
              <div className={cn(
                ...
                <div className="space-y-4">
                  ...
                  {selectedRequest.currentStatus === "Closed" && (
                    <div className="flex gap-2 items-center text-xs text-neutral-400 justify-center py-2 italic font-semibold">
              ...
              <div className="space-y-3.5">
                ...
                <div className="grid grid-cols-2 gap-4 text-xs">
                  ...
                  <div>
                    ...
                    <p className="font-semibold mt-0.5">{selectedRequest.user?.email}</p>
                  </div>
                  <div>
                    <p className="text-neutral-500">Registered Phone</p>
                    <p className="font-semibold mt-0.5">{selectedRequest.user?.phoneNumber || "N/A"}</p>
                  </div>
                  ...
                  {selectedRequest.user?.role === "DISTRIBUTOR" && (
                    <>
                      <div>
                        ...
                        <p className="font-bold mt-0.5 text-indigo-400">{selectedRequest.user.companyName || "N/A"}</p>
                      </div>
                      <div>
                        <p className="text-neutral-500">GSTIN</p>
                        <p className="font-mono font-bold mt-0.5 text-indigo-400">{selectedRequest.user.gstin || "N/A"}</p>
                      </div>
                      <div>
                        <p className="text-neutral-500">Distributor ID</p>
                        <p className="font-mono font-semibold mt-0.5 text-neutral-400">{selectedRequest.user.id}</p>
                      </div>
                    </>
                  )}
                </div>
              ...
              <div className="space-y-3.5">
                ...
                <div className="grid grid-cols-2 gap-4 text-xs">
                  <div>
                    ...
                    <p className="font-bold mt-0.5">{selectedRequest.product?.name}</p>
                  </div>
                  <div>
                    <p className="text-neutral-500">SKU Code</p>
                    <p className="font-mono font-bold mt-0.5">{selectedRequest.product?.sku || "N/A"}</p>
                  </div>
                  <div>
                    <p className="text-neutral-500">Variant Group</p>
                    <p className="font-semibold mt-0.5">{selectedRequest.product?.variantGroup || "N/A"}</p>
                  </div>
                  <div>
                    <p className="text-neutral-500">Internal Product ID</p>
                    <p className="font-mono text-neutral-450 mt-0.5">{selectedRequest.product?.id}</p>
                  </div>
                  <div>
