...
exports.calculateOrderPricing = exports.runPricingEngine = exports.deleteOffer = exports.updateOffer = exports.createOffer = exports.getOfferById = exports.getOffers = void 0;
const db_1 = require("../utils/db");
// Helper function to extract a product's brand from specs or fallback
const getProductBrand = (product) => {
    if (product.eyebrow)
        return product.eyebrow.trim().toLowerCase();
    if (product.specs && Array.isArray(product.specs)) {
        const brandSpec = product.specs.find((s) => s.label?.toLowerCase() === "brand" ||
...
const getOfferById = async (req, res) => {
    try {
        ...
        const offer = await db_1.prisma.offer.findUnique({
            where: { id },
        });
        if (!offer) {
...
const createOffer = async (req, res) => {
    try {
        ...
        const { name, code, description, offerType, bannerImage, priority, status, startDate, endDate, applyAutomatically, stackable, maxUsageLimit, usagePerCustomer, displayBadgeText, badgeColor, termsConditions, configuration, } = req.body;
        if (!name || !code || !offerType || !startDate || !endDate) {
            res.status(400).json({ success: false, message: "Missing required offer fields" });
            return;
        }
        // Verify code uniqueness
        const existing = await db_1.prisma.offer.findUnique({ where: { code } });
        if (existing) {
...
const updateOffer = async (req, res) => {
    try {
        ...
        const id = req.params.id;
        const { name, code, description, offerType, bannerImage, priority, status, startDate, endDate, applyAutomatically, stackable, maxUsageLimit, usagePerCustomer, displayBadgeText, badgeColor, termsConditions, configuration, } = req.body;
        const existingOffer = await db_1.prisma.offer.findUnique({ where: { id } });
        if (!existingOffer) {
            res.status(404).json({ success: false, message: "Offer not found" });
            return;
        }
        const offer = await db_1.prisma.offer.update({
            where: { id },
            data: {
                name,
                code,
...
const deleteOffer = async (req, res) => {
    try {
        ...
        const id = req.params.id;
        await db_1.prisma.offer.delete({ where: { id } });
...
const isOfferApplicable = (offer, couponCode) => {
    if (offer.status !== "ACTIVE")
        return false;
    // 1. If set to apply automatically, it's always applicable
    if (offer.applyAutomatically)
        return true;
    ...
    if (couponCode) {
        ...
        const configCouponCode = String(offer.configuration?.couponCode || "").trim().toLowerCase();
...
const runPricingEngine = async (user, items, couponCode) => {
    // A. Fetch the products details
    const productIds = items.map((i) => i.productId);
    const dbProducts = await db_1.prisma.product.findMany({
    ...
    const now = new Date();
    const activeOffers = await db_1.prisma.offer.findMany({
        where: {
            status: "ACTIVE",
            startDate: { lte: now },
            endDate: { gte: now },
    ...
    const appliedOffersGlobal = [];
    const freeItemsToAdd = [];
    // Helper to evaluate flat vs percentage discount values
    const evaluateDiscount = (type, value, currentPrice, maxCap) => {
        let amt = 0;
        if (type === "PERCENTAGE") {
            amt = currentPrice * (value / 100);
            if (maxCap)
                amt = Math.min(amt, maxCap);
        }
        else {
            amt = value;
        }
    ...
    const canApplyOffer = (item, offer) => {
        if (hasAppliedNonStackable)
            return false;
        if (item.appliedOffers.some((o) => !o.stackable))
            return false;
        if (!offer.stackable && item.appliedOffers.length > 0)
            return false;
        return true;
    };
    // 1. PRODUCT_DISCOUNT
    const productOffers = activeOffers.filter((o) => o.offerType === "PRODUCT_DISCOUNT" && isOfferApplicable(o, couponCode));
        ...
        for (const item of calculatedItems) {
            if (targetIds.includes(item.productId) && item.quantity >= minPurchaseQty) {
                if (canApplyOffer(item, offer)) {
                    const disc = evaluateDiscount(config.discountType, Number(config.discountValue || 0), item.unitPrice, config.maxDiscountAmount);
    ...
    // 2. CATEGORY_DISCOUNT
    const categoryOffers = activeOffers.filter((o) => o.offerType === "CATEGORY_DISCOUNT" && isOfferApplicable(o, couponCode));
    ...
    // 3. BRAND_DISCOUNT
    const brandOffers = activeOffers.filter((o) => o.offerType === "BRAND_DISCOUNT" && isOfferApplicable(o, couponCode));
    ...
    // 4. QUANTITY_DISCOUNT
    const qtyOffers = activeOffers.filter((o) => o.offerType === "QUANTITY_DISCOUNT" && isOfferApplicable(o, couponCode));
        ...
        for (const item of calculatedItems) {
            ...
            if (matchesProduct && matchesCategory) {
                const applicableRules = rules
                    .filter((r) => item.quantity >= r.minQty)
                    .sort((a, b) => b.minQty - a.minQty);
                if (applicableRules.length > 0 && canApplyOffer(item, offer)) {
                    const bestRule = applicableRules[0];
                    const disc = evaluateDiscount(bestRule.discountType, Number(bestRule.discountValue || 0), item.unitPrice);
                    item.unitPrice = Math.max(0, item.unitPrice - disc);
                    item.appliedOffers.push({
                        offerId: offer.id,
                        name: offer.name,
    ...
    const genericDiscounts = activeOffers.filter((o) => (o.offerType === "FLAT_DISCOUNT" || o.offerType === "PERCENTAGE_DISCOUNT") && isOfferApplicable(o, couponCode));
        ...
        if (offer.offerType === "FLAT_DISCOUNT") {
            ...
            for (const item of calculatedItems) {
                ...
                if (matchesProduct && matchesCategory) {
                    const isPurchaseValueMet = !config.minPurchaseValue || (item.unitPrice * item.quantity) >= Number(config.minPurchaseValue);
                    if (isPurchaseValueMet && canApplyOffer(item, offer)) {
                        const disc = evaluateDiscount("PERCENTAGE", Number(config.percentageValue || 0), item.unitPrice, config.maxDiscountCap);
                        item.unitPrice = Math.max(0, item.unitPrice - disc);
                        item.appliedOffers.push({
                            offerId: offer.id,
                            name: offer.name,
                ...
                if (config.offerType === "SAME" || config.buyProductId === config.getProductId) {
                    ...
                    if (rawFreeQty > 0) {
                        ...
                        if (getItem) {
                            ...
                            const promoProduct = await db_1.prisma.product.findUnique({
    ...
    calculatedItems.forEach((item) => {
        const bogoSame = item.appliedOffers.find((o) => o.bogoDescription && o.bogoDescription.includes("BOGO applied"));
        if (bogoSame) {
    ...
    const cartValueOffers = activeOffers.filter((o) => o.offerType === "CART_VALUE_DISCOUNT" && isOfferApplicable(o, couponCode));
    for (const offer of cartValueOffers) {
        ...
        if (subtotal >= Number(config.minCartValue || 0)) {
            ...
            if (catIds.length > 0 || prodIds.length > 0) {
                isEligible = calculatedItems.some((i) => catIds.includes(i.category) || prodIds.includes(i.productId));
            }
            if (isEligible) {
                const disc = evaluateDiscount(config.discountType, Number(config.discountValue || 0), subtotal, config.maxDiscountCap);
    ...
    // 11. COUPON_DISCOUNT
    const couponOffers = activeOffers.filter((o) => o.offerType === "COUPON" && isOfferApplicable(o, couponCode));
    for (const offer of couponOffers) {
        ...
        if (user && config.eligibility !== "ALL") {
            const userOrders = await db_1.prisma.order.findMany({
    ...
    if (user) {
        const userOrdersCount = await db_1.prisma.order.count({ where: { userId: user.id } });
    ...
    // 13. LOYALTY_DISCOUNT
    if (user) {
        const userOrders = await db_1.prisma.order.findMany({ where: { userId: user.id } });
        const currentTier = getUserTier(userOrders);
        const loyaltyOffers = activeOffers.filter((o) => o.offerType === "LOYALTY" &&
            o.configuration.membershipTier === currentTier &&
    ...
    const freeShippingProductIds = [];
    for (const item of calculatedItems) {
        const dbProduct = dbProducts.find((p) => p.id === item.productId);
        if (dbProduct) {
            const hasFreeShippingBadge = dbProduct.badge?.toLowerCase().includes("free shipping") ||
                dbProduct.badge?.toLowerCase().includes("free delivery");
            const hasFreeShippingSpec = Array.isArray(dbProduct.specs) &&
                dbProduct.specs.some((s) => s.label?.toLowerCase().includes("shipping") &&
    ...
    const allItemsHaveFreeShipping = calculatedItems.length > 0 &&
    ...
    const freeShippingOffers = activeOffers.filter((o) => o.offerType === "FREE_SHIPPING" && isOfferApplicable(o, couponCode));
    for (const offer of freeShippingOffers) {
        ...
        if (subtotal >= Number(config.minOrderValue || 0)) {
            ...
            if (cats.length > 0 || prods.length > 0) {
                isEligible = calculatedItems.some((i) => cats.includes(i.category) || prods.includes(i.productId));
            }
            if (isEligible) {
    ...
    const amountNeededForFreeDelivery = deliveryCharges > 0 ? Math.max(0, FREE_DELIVERY_THRESHOLD - subtotal) : 0;
    const sumProductsDiscounts = calculatedItems.reduce((sum, item) => sum +
        (item.originalPrice * item.quantity - item.totalPrice), 0);
    ...
    calculatedItems.forEach((item) => {
        item.appliedOffers.forEach((itemOffer) => {
...
exports.runPricingEngine = runPricingEngine;
const calculateOrderPricing = async (req, res) => {
    try {
        const user = req.user;
        ...
        const result = await (0, exports.runPricingEngine)(user, items, couponCode);
